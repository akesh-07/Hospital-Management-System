...
export const DrugInventory: React.FC = () => {
  ...
  const filteredDrugs = drugs.filter(drug => {
    const matchesSearch = drug.drugName.toLowerCase().includes(searchTerm.toLowerCase()) ||
    ...
    const matchesStock = stockFilter === 'all' ||
                        ...
                        (stockFilter === 'low' && drug.stockQuantity > 0 && drug.stockQuantity <= 20) ||
    ...
    <div className="bg-white rounded-lg border border-gray-200 p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <select
          ...
          onChange={(e) => onChange({ ...drug, dosageForm: e.target.value as Drug['dosageForm'] })}
          className="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent"
        >
        ...
        <input
          ...
          onChange={(e) => onChange({ ...drug, stockQuantity: parseInt(e.target.value) || 0 })}
          className="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent"
        />
        <input
          type="number"
          ...
          onChange={(e) => onChange({ ...drug, unitPrice: parseFloat(e.target.value) || 0 })}
          className="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent"
        />
        <input
          type="text"
        ...
        <input
          ...
          onChange={(e) => onChange({ ...drug, manufacturingDate: e.target.value })}
          className="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent"
        />
      </div>
        ...
        {editingDrug && (
          <div className="mb-6">
            <DrugForm
              ...
              onSubmit={() => {
                setDrugs(drugs.map(d => d.id === editingDrug.id ? editingDrug as Drug : d));
                setEditingDrug(null);
              }}
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-gray-200">
                  <th className="text-left py-3 px-4 font-medium text-gray-700">Medicine Details</th>
                  ...
                  <tr key={drug.id} className="border-b border-gray-100 hover:bg-gray-50">
                    <td className="py-4 px-4">
                      <div>
                        ...
                        <p className="text-xs text-gray-500">{drug.strength} • {drug.dosageForm}</p>
                      </div>
                    </td>
                    <td className="py-4 px-4">
                    ...
                    <td className="py-4 px-4">
                      <div className="flex items-center space-x-2">
                        <span className="font-medium text-gray-900">{drug.stockQuantity}</span>
                    ...
                    <td className="py-4 px-4">
                      <span className="font-medium text-gray-900">${drug.unitPrice.toFixed(2)}</span>
                    </td>
                    <td className="py-4 px-4">
                      <div className="flex items-center space-x-2">
                    ...
                    <td className="py-4 px-4">
                      <span className="text-sm text-gray-600">{drug.supplierInfo}</span>
                    </td>
                    <td className="py-4 px-4">
                      <div className="flex items-center space-x-2">
                        <button
        ...
        <div className="mt-6 bg-white rounded-lg border border-gray-200 p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="border-2 border-dashed border-gray-300 rounded-lg p-4 text-center hover:border-green-400 transition-colors cursor-pointer">
              ...
              <p className="text-sm text-gray-600 mb-1">Upload CSV/Excel</p>
